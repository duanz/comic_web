...
class ChapterDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ("id", "book_id", "title", "number", "order", "active", "create_at", 
                  "origin_addr", "relate_chapter_id")
    
    def get_relate_chapter_id(self, obj):
        query_set = BookAdminModels.Chapter.normal.filter(book_id=obj.book_id).values("id")
...
class BookDetailSerializer(serializers.ModelSerializer):
    create_at = serializers.DateTimeField(format="%Y-%m-%d %H:%I:%S", required=False)
    author = ComicAdminSerializers.AuthorSerializer(read_only=True)
